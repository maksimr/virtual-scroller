...
describe('VirtualScroller', function() {
  ...
  it('should correctly scroll to if scroll node has offset between viewport node', function() {
    ...
    expect(virtualScroller.scrollPosition()).toEqual(0);

    const scrollTop = 5;
    virtualScroller.scrollTo(scrollTop);
    scroll(scrollElement);

    expect(virtualScroller.scrollPosition()).toEqual(scrollTop);
  ...
  it('should change scrollbar size if we can not fit to current scrollbar size', function() {
    const VISIBLE_ITEMS_COUNT = 5;
    const ITEMS_COUNT = 6;
    ...
    expect(scrollHeight(viewportElement)).toEqual(ITEMS_COUNT * DEFAULT_ITEM_SIZE);

    scrollTo(viewportElement, ITEM_SIZE);
    expect(scrollHeight(viewportElement)).toEqual(ITEMS_COUNT * ITEM_SIZE);
  });

  function scrollHeight(element) {
    return parseFloat(element.firstChild.style.height);
  }
