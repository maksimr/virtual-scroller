...
export class VirtualScroller {
  ...
  updateRenderedRange(range) {
    ...
    if (!isResize) {
      for (let i = range.start; i <= range.end; i++) {
        if (
          this.rendered[i].offset <= this.scrollbar.scrollTop &&
          this.rendered[i].offset + this.sizeManager.getHopefulSize(this.rendered[i]) > this.scrollbar.scrollTop
        ) {
          this.anchor = {index: i, offset: this.rendered[i].offset};
        }
      }
    } else if (this.anchor && this.rendered[this.anchor.index]) {
      if (this.anchor.offset !== this.rendered[this.anchor.index].offset) {
        this.scrollbar.scrollTo(this.scrollbar.scrollTop + (this.rendered[this.anchor.index].offset - this.anchor.offset));
        this.anchor.offset = this.rendered[this.anchor.index].offset;
      }
    } else {
      this.anchor = null;
    }
