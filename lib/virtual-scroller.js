...
export class VirtualScroller {
  ...
  updateRenderedRange(range) {
    const anchorNode = this.findAnchorNode();
    const addedElements = [];
    ...
    if (this.range.start > range.start && this.range.start <= range.end) {
      ...
      if (offset < 0) {
        this.scrollTo(this.scrollbar.scrollTop - offset);
      }
    }
    ...
    for (let i = range.start; i <= range.end; i++) {
      const renderedElement = this.rendered[i];
      if (renderedElement === anchorNode) {
        scrollToAnchor = offset - renderedElement.offset;
      }
    ...
    if (scrollToAnchor) {
      this.scrollTo(this.scrollbar.scrollTop + scrollToAnchor);
    }
  ...
  findAnchorNode() {
    const index = Object.keys(this.rendered).find((i) => {
      const renderedElement = this.rendered[i];
      return renderedElement.offset <= this.scrollbar.scrollTop && renderedElement.offset + this.sizeManager.getHopefulSize(renderedElement) > this.scrollbar.scrollTop;
    });
