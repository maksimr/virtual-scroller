...
export class VirtualScroller {
  ...
  updateRenderedRange(range) {
    ...
    if (this.range.start > range.start && this.range.start <= range.end) {
      ...
      if (offset < 0) {
        this.scrollTo(this.scrollTop() - offset);
      }
    }
    ...
    if (scrollToAnchor) {
      this.scrollTo(this.scrollTop() + scrollToAnchor);
    }
  ...
  findAnchorNode() {
    const index = Object.keys(this.rendered).find((i) => {
      const renderedElement = this.rendered[i];
      return renderedElement.offset <= this.scrollTop() && renderedElement.offset + this.sizeManager.getHopefulSize(renderedElement) > this.scrollTop();
    });
