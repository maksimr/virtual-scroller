  ...
  sync() {
    ...
    for (let i = startIndex; i <= endIndex; i++) {
      if (!this.rendered[i]) {
        ...
        addResizeListener(itemNode, () => {
          this.sizeManager.remove(itemNode);
          this.scheduleSync();
        });
...
function createResizeObserver() {
  if (!createResizeObserver.resizeObserver) {
    ...
    const style = 'position: absolute; height: 100%; width: 100; opacity: 0; visibility: hidden; z-index: -1; overflow: hidden; contain: paint size style; pointer-events: none;';
