...
export class VirtualScroller {
  ...
  calcRange() {
    ...
    for (let i = this.range.start; i <= this.range.end; i++) {
      ...
      if (renderedElement) {
        start = (renderedElement && renderedElement.offset + this.sizeManager.getHopefulSize(renderedElement) > startOffset) ? Math.min(i, start) : start;
        end = (renderedElement.offset < endOffset && renderedElement.offset + this.sizeManager.getHopefulSize(renderedElement) > endOffset) ? i : end;
      }
    }
